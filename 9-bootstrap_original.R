...
if (exposure == 'antidepressant' &
    analysis == 'young') {
  variables <- variables[!variables %in% c('pacemaker', 'valvular_heart_disease', 'lvh')]
}
...
if (exposure == 'antidepressant') {
  ...
  p_uncens_predictors_pooled <- as.formula(paste(c(p_uncens_predictors, 'dec'), collapse = '+'))
} 
...
bs <- function(data, indices) {
  ...
  result <- c(
    ...
    IRR = (ir_results[4,]),
    IRD = (ir_results[5,]),
    IR_stab = (ir_results[6,]),
    IR_ref_stab = (ir_results[7,]),
    IR_comp_stab = (ir_results[8,]),
    IRR_stab = (ir_results[9,]),
    IRD_stab = (ir_results[10,])
  )
  ...
  saveRDS(bs_results_manual, file = paste(path_final_res, 'bs_results_manual.rds', sep='/'))
  ...
  #saveRDS(bs_results_manual_t0, file = paste(path_final_res, 'bs_results_manual_t0.rds', sep='/'))
...
# incidence rates(rows): 10 (IR, IR_ref, IR_comp, ...)
# anlaysis types: 12 (itt, itt.iptw, at, ...)
# total columns per measure: 12*10 = 120
...
#   rep('IRD.', each = 12),
#   rep('IR_stab.', each = 12),
#   rep('IR_ref_stab.', each = 12),
#   rep('IR_comp_stab.', each = 12),
#   rep('IRR_stab.', each = 12),
#   rep('IRD_stab.', each = 12)
# )
# 
# bs_col_names <- rep(col_names, times = 10)
# bs_col_names <- paste(tags, bs_col_names, sep = '')
# 
# saveRDS(bs_col_names, file = paste(path_intermediate_res_main, 'bs_col_names.rds', sep = '/'))
...
bs_col_names <- readRDS(file = paste(path_intermediate_res_main, 'bs_col_names.rds', sep='/'))
bs_results_manual <- data.frame(matrix(nrow = 1, ncol = length(bs_col_names)))
names(bs_results_manual) <- bs_col_names

# time_to_start <- force_tz(ymd_hms("2024-11-13 10:30:00"), tzone = "America/Montreal")
...
saveRDS(bs_results_manual, file = paste(path_final_res, 'bs_results_manual.rds', sep='/'))
saveRDS(bs_results_boot, file = paste(path_final_res, 'bs_results_boot.rds', sep='/'))
...
bs_results_boot <- readRDS (file = paste(path_final_res, 'bs_results_boot.rds', sep = '/'))
...
saveRDS(bs_ci_boot, file = paste(path_final_res, 'bs_ci_boot.rds', sep='/'))
...
saveRDS(ir_ci_boot, paste(path_final_res, 'ir_ci_boot.rds', sep ='/'))
write_xlsx(ir_ci_boot, paste(path_final_res, 'ir_ci_boot.xlsx', sep ='/'))

#### BUILD BOOTSTRAPPED CIs - USING MANUAL SAVE ####

# if results saved in different chunks can combine them
save1 <- readRDS(paste(path_final_res, 'bs_results_manual1.rds', sep='/'))
save2 <- readRDS(paste(path_final_res, 'bs_results_manual2.rds', sep='/'))
results <- rbind(save1, save2)

results %<>% filter(!is.na(IR.itt))
saveRDS(results, file = paste(path_final_res, 'bs_results_manual_final.rds', sep = '/'))
# results <- results[-1,]

# results <- readRDS(paste(path_final_res, 'bs_results_manual.rds', sep='/'))
results_t0 <- readRDS(paste(path_final_res, 'bs_results_manual_t0.rds', sep='/'))
...
saveRDS(bs_ci_manual, file = paste(path_final_res, 'bs_ci_manual.rds', sep='/'))
...
write_xlsx(ir_ci_manual, paste(path_final_res, 'ir_ci_manual.xlsx', sep ='/'))
